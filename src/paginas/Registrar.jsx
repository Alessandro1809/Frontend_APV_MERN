...
export const Registrar = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if ([nombreLimpio, emailLimpio, password, confirmaPassword].includes("")) {
      setAlerta({
        mensaje: "Parece que alguno de los campos esta vacio",
        error: true,
      });
      return;
    }
    ...
    if (password.length < 6) {
      setAlerta({
        mensaje:
          "El password que ingresaste es muy corto, agrega mas de 6 caracteres",
        error: true,
      });
      return;
    }
    ...
    try {
      ...
      setAlerta({
        mensaje: "Creado Correctamente, revisa tu correo",
        error: false,
      });
    } catch (error) {
      setAlerta({
